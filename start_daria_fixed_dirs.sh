...
echo "====== Starting DARIA Interview Tool with Directory Structure Fix ======"
...
echo "Running directory structure fix..."
...
if [[ $(echo "$PY_VERSION >= 3.13" | bc -l) -eq 1 ]]; then
    ...
    if [ -f "fix_pydantic_forward_refs.py" ]; then
        ...
        STARTUP_CMD="python fix_pydantic_forward_refs.py fix_directory_structure.py run_interview_api.py --use-langchain --port 5025"
    else
        echo "No pydantic patch found, just using directory fix..."
        STARTUP_CMD="python fix_directory_structure.py run_interview_api.py --use-langchain --port 5025"
    fi
else
