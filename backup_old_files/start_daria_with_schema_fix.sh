...
echo "====== Starting DARIA Interview Tool with Schema Compatibility Fix ======"
...
chmod +x fix_schema_compatibility.py
...
# Then apply schema compatibility fixes
echo "Fixing schema compatibility issues..."
python fix_schema_compatibility.py

if [ $? -ne 0 ]; then
    echo "⚠️ Schema compatibility fixes completed with some errors, but continuing..."
else
    echo "✅ Schema compatibility fixes applied successfully"
fi
