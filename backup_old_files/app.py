...
# Import and register the issues blueprint
try:
    from routes.issue_routes import bp as issues_bp
    app.register_blueprint(issues_bp)
    print("Successfully registered Issues blueprint")
except Exception as e:
    print(f"Failed to register Issues blueprint: {str(e)}")
    # Continue without the issues functionality
...
def archive():
    ...
    try:
        ...
        return render_template('error.html', error="Failed to load interviews")
...
def interview_details(session_id):
    """Display details for a specific interview session."""
    try:
        # Load the session file from the sessions directory
        session_file = Path('data/interviews/sessions') / f"{session_id}.json"
        if not session_file.exists():
            return render_template('error.html', error="Interview session not found")
        ...
        logger.error(f"Error loading interview details for {session_id}: {str(e)}")
        return render_template('error.html', error="Failed to load interview details")
