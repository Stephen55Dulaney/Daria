...
class ProcessedInterviewStore:
    def __init__(self, base_dir: str = "data/interviews/sessions"):
        """Initialize the store with the base directory for processed interviews.
        
        Args:
            base_dir: Base directory containing interview data. Defaults to data/interviews/sessions.
        """
        logger.info(f"Initializing ProcessedInterviewStore with base_dir: {base_dir}")
        self.base_dir = base_dir
        self.processed_dir = os.path.join(base_dir, "processed")
        self.default_project_name = "Unknown Project"
        
        # Ensure directories exist
        os.makedirs(self.base_dir, exist_ok=True)
        os.makedirs(self.processed_dir, exist_ok=True)
        logger.info(f"Created/verified directories: {self.base_dir}, {self.processed_dir}")
        ...
        try:
            ...
            logger.error(f"Warning: Could not initialize sentence transformer model: {e}")
            self.model = None
    ...
    def _get_interview_path(self, interview_id: str) -> str:
        return os.path.join(self.processed_dir, f"{interview_id}.json")
    ...
    def save_interview(self, interview_id: str, data: Dict) -> None:
        ...
        logger.info(f"Saving interview {interview_id} to {file_path}")
        with open(file_path, 'w') as f:
            json.dump(data, f, indent=2)

    def load_interview(self, interview_id: str) -> Optional[Dict]:
        """Load an interview from either the processed directory or the main sessions directory."""
        # First try the processed directory
        processed_path = os.path.join(self.processed_dir, f"{interview_id}.json")
        logger.info(f"Attempting to load interview {interview_id} from {processed_path}")
        if os.path.exists(processed_path):
            try:
                with open(processed_path, 'r') as f:
                    data = json.load(f)
                    data['_file_path'] = processed_path
                    logger.info(f"Successfully loaded processed interview {interview_id}")
                    return data
            except Exception as e:
                logger.error(f"Error loading processed interview {interview_id}: {e}")
        
        # If not found in processed, try the main sessions directory
        session_path = os.path.join(self.base_dir, f"{interview_id}.json")
        logger.info(f"Attempting to load interview {interview_id} from {session_path}")
        if os.path.exists(session_path):
            try:
                with open(session_path, 'r') as f:
                    data = json.load(f)
                    data['_file_path'] = session_path
                    logger.info(f"Successfully loaded session interview {interview_id}")
                    return data
            except Exception as e:
                logger.error(f"Error loading session interview {interview_id}: {e}")
        
        logger.warning(f"Interview {interview_id} not found in either location")
        return None
    ...
    def semantic_search(self, query: str, k: int = 10) -> List[Dict]:
        ...
        logger.info(f"Performing semantic search for query: '{query}' with limit {k}")
        results = []
        search_criteria = self._extract_search_criteria(query)
        
        if not self.model:
            logger.error("Cannot perform semantic search - model not initialized")
            return []
        ...
        for filename in files:
            ...
            if not interview_data or 'chunks' not in interview_data:
                logger.warning(f"No valid data found for interview {interview_id}")
                continue
            ...
            for chunk in interview_data['chunks']:
                ...
                if similarity > 0.3 and (emotion_match or theme_match):
                    ...
                    logger.info(f"Found matching chunk with similarity {similarity:.2f}")
    ...
    def _create_search_result(self, interview_data: Dict, chunk: Dict, similarity: float = 1.0) -> Dict:
        ...
        logger.debug(f"Created search result: {result}")
        return result
    ...
    def search(self, query: str, search_type: str = 'text', limit: int = 10) -> List[Dict]:
        ...
        logger.info(f"Performing {search_type} search for query: '{query}' with limit {limit}")
        
        if search_type == 'semantic':
            if not self.model:
                logger.error("Semantic search requested but model not initialized")
                return []
