...
def save_interview():
    ...
    try:
        ...
        logger.info(f"Received form data (type: {type(data)}): {data}")
        
        # Check if we're receiving nested data structure
        if isinstance(data, dict) and 'project' in data and 'researcher' in data and 'interviewee' in data:
            logger.info("Received nested form structure - reformatting")
            ...
            new_data = {
                ...
                'participant_name': interviewee_data.get('name', 'Anonymous'),
                'role': interviewee_data.get('role', ''),
                'experience_level': interviewee_data.get('experience', ''),
                'department': interviewee_data.get('department', ''),
                'interviewer_name': researcher_data.get('name', 'Unknown')
            }
            
            data = new_data
            logger.info(f"Reformatted data: {data}")
        ...
        logger.info(f"Extracted key fields: project_name={project_name}, interview_type={interview_type}")
        ...
        metadata = {
            ...
            'researcher': {
                'name': data.get('interviewer_name', 'Unknown')
            }
        }

        # Generate the interview prompt
        try:
            from daria_interview_tool.daria_resources import get_interview_prompt
            interview_prompt = get_interview_prompt(interview_type, project_name, project_description)
            logger.info(f"Generated interview prompt: {interview_prompt[:100]}...")
        except Exception as e:
            logger.error(f"Error generating interview prompt: {str(e)}")
            return jsonify({'error': f'Failed to generate interview prompt: {str(e)}'}), 500

        # Initialize the global dictionaries if they don't exist
        global interview_prompts, conversations
        if 'interview_prompts' not in globals():
            interview_prompts = {}
        if 'conversations' not in globals():
            conversations = {}
        ...
        interview_prompts[project_name] = {
            ...
            'type': interview_type,
            'form_data': data  # Store the entire form data
        }
        
        # Initialize conversation for this project
...
def interview(project_name):
    try:
        if request.method == 'GET':
            ...
            prompt_data = interview_prompts[project_name]
            
            # Render the interview template
            return render_template('interview.html', 
                                  project_name=project_name, 
                                  prompt=prompt_data,
                                  interview_type=prompt_data.get('type', 'Interview'))
        else:  # POST request
            data = request.get_json()
            ...
            prompt_data = interview_prompts.get(project_name)
            if not prompt_data:
                return jsonify({'error': 'Interview prompt not found'}), 404
            
            # Get the interview type from the prompt
            interview_type = prompt_data.get('type', "Application Interview")
            project_description = prompt_data.get('project_description', '')
            
            # Generate system message based on interview type
            try:
                from daria_interview_tool.daria_resources import get_interview_prompt
                system_message = get_interview_prompt(interview_type, project_name, project_description)
            except Exception as e:
                logger.error(f"Error generating system message: {str(e)}")
                return jsonify({'error': f'Failed to generate system message: {str(e)}'}), 500
            ...
            try:
                response = llm.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": system_message},
                        *conversations[project_name]['messages'][1:]  # Skip the system message
                    ],
                    temperature=0.7
                )
...
def create_journey_map():
    ...
    try:
        ...
        # Generate journey map JSON
        logger.info("Generating journey map JSON")
        journey_map_json = generate_journey_map_json(interviews)
        logger.info(f"Generated JSON content")
        
        return jsonify(journey_map_json)
    except Exception as e:
        logger.error(f"Error creating journey map: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({'error': str(e)}), 500

def generate_journey_map_json(interviews):
    """Generate JSON for the journey map based on interview data."""
    # Create the journey map structure
    journey_map = {
        "journey_map": {
            "persona_id": str(uuid.uuid4()),  # Generate a unique ID
            "persona_name": "User",  # Default name, can be updated based on interview data
            "project_name": "User Journey",  # Default name, can be updated based on interview data
            "created_at": datetime.now().isoformat(),
            "stages": [],
            "experience_curve": []
        }
    ...
    # Extract data from each interview
    for interview in interviews:
        # Get persona data if available
        if 'persona_data' in interview:
            persona_data = interview['persona_data']
            journey_map['journey_map']['persona_name'] = persona_data.get('name', 'User')
            journey_map['journey_map']['project_name'] = interview.get('project_name', 'User Journey')
        ...
        for stage in stages:
            stage_data = {
                ...
                "storyboard_image": stage.get('storyboard_image', ''),
                "user_actions": stage.get('user_actions', []),
            ...
            journey_map['journey_map']['stages'].append(stage_data)
            
            # Add to experience curve
            for emotion in stage.get('emotions', []):
                journey_map['journey_map']['experience_curve'].append({
                    "stage": stage.get('stage_name', ''),
                    "emotion": emotion.get('emotion', ''),
                    "intensity": emotion.get('intensity', 3)  # Default intensity
                })
