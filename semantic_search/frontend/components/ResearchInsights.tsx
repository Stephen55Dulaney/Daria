...
interface Insight {
  type: 'BEHAVIORAL' | 'ATTITUDINAL' | 'PAIN_POINT' | 'OPPORTUNITY' | 'THEME';
  content: string;
  confidence: number;
              ...
              >
                {insight.context.sub_themes && (
                  <Box mt={2}>
                    ...
                    <Box display="flex" gap={1} flexWrap="wrap">
                      {insight.context.sub_themes.map((theme, idx) => (
                ...
                {insight.supporting_quotes.length > 0 && (
                  <Box mt={2}>
                    ...
                    {insight.supporting_quotes.map((quote, idx) => (
                      <Card key={idx} variant="outlined" sx={{ mt: 1, bgcolor: 'grey.50' }}>
                        <CardContent>
                          <Typography variant="body2" color="text.secondary" gutterBottom>
                            {quote.speaker} at {quote.timestamp}
                          </Typography>
